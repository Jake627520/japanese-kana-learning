...
export const N3_TOPICS_BATCH1: JlptTopic[] = [
  { id: 'jpg_n3-wake-ga-nai', type: 'grammar', subject: '日本語', domain: '文法',
    ...
    description: '根據常理或客觀事實判斷「絕不可能」。接續：動詞・い形容詞普通形（辭書形／ない形／た形皆可）＋わけがない、な形容詞＋な、名詞＋の。最容易混的是 わけではない（並非完全…，部分否定），差一個字意思差很多。',
  ...
  { id: 'jpg_n3-tame-ni', type: 'grammar', subject: '日本語', domain: '文法',
    ...
    description: '書面語的客觀原因（事故のために電車が止まった）。與表示目的的ために同形，判準看前後兩件事的關係：後件是為了讓前件成立才做的→目的；前件先發生、後件跟著發生→原因。光看前件分不出來（試験のために勉強する＝目的／試験のために休みだ＝原因）。',
    evidence: ['能用前後件的關係判斷ために表原因還是目的', '知道「後件非意志事件⇒原因」是單向線索，反推不成立'] },
